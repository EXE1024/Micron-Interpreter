...
class MVE:
    ...
    def start(self):
        ...
        version = ttk.Label(versions, text=f"Executor Version: {self.ver}")
    ...
    def get_intversion(self):
        ...
        for f in files:
            if f.startswith("micron") and f.endswith(".py"):
                ...
                if new == "2.0" or new == "1.1.2":
                    return f"Interpreter version: {new} [Stable]"
                elif new == "":
                    unk = False
                else: 
                    incompatible = new
        ...
        try:
            ...
            if os.name == 'nt':
                ...
                if not synced:
                    process.terminate()
                    process.wait()
                    messagebox_answer = messagebox.askyesno(" Security Alert ", "The detected interpreter did not synchronize correctly with the system. Press Yes to continue")

                    if not messagebox_answer:
                        return
